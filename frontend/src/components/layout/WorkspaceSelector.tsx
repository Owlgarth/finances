...
export default function WorkspaceSelector({ onOpenSettings, collapsed = false }: WorkspaceSelectorProps) {
  ...
  return (
    <div className="relative" ref={dropdownRef}>
      ...
      {/* Create-workspace modal (mount-per-use): the row above closes this
          dropdown before opening it, so Escape only ever faces one layer. */}
      {createOpen && <CreateWorkspaceForm onClose={() => setCreateOpen(false)} />}
    </div>
  )
}
