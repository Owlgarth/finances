...
interface LegalDocPageProps {
  fetcher: () => Promise<LegalDoc>;
  title: string;
  failureText: string;
}

export default function LegalDocPage({ fetcher, title, failureText }: LegalDocPageProps) {
    ...
    <div className="min-h-screen bg-surface py-12 px-4 sm:px-6 lg:px-8">
      <div className="max-w-3xl mx-auto">
        ...
        <div className="bg-surface rounded-sm p-8 border border-border">
          ...
          {error && (
            <p className="text-negative">{failureText}</p>
          )}
            ...
            <div className="prose-legal">
              <ReactMarkdown
                components={{
                  ...
                  code: ({ children }) => (
                    <code className="bg-surface-hover px-1 rounded-none text-sm">{children}</code>
                  ),
                  a: ({ href, children }) => (
