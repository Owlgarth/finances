...
interface CurrencySetFieldProps {
  ...
  placeholder?: string
  /** Helper line under the picker, pre-translated by the caller. Budget
   *  contexts pass nothing (the translated budget-table default); other
   *  surfaces pass their own context-appropriate copy. */
  helperText?: string
...
/**
 * Ordered currency-set field (MultiSelect, helper line, optional manage
 * link, reordering list). Shared component: budgets read options ambiently
 * from useEnabledCurrencies(); pre-workspace forms (workspace creation,
 * register, account reset) pass explicit options via `currencies` plus
 * their own primaryLabel/placeholder (and helperText where the budget
 * default does not fit). `value` is the single order-significant source
 * of truth (index 0 = shown first): the MultiSelect appends new picks at
 * the end and removes deselections; only the arrow list reorders. Zero
 * useState/useEffect - structurally lint-quiet.
 */
export default function CurrencySetField({
  ...
  const resolvedHelperText = helperText ?? t('currencySet.budgetHelper')
  ...
  return (
    <div>
      ...
      <MultiSelect
        ...
        className="w-full"
      />
      <p className="mt-1 text-[11px] text-text-muted">{resolvedHelperText}</p>
            ...
            <li key={code} className="flex items-center justify-between px-3 py-2">
              ...
              <span className="flex items-center gap-2">
                {/* Real padded hit areas, never the shared hit-area utility:
                     the arrows are adjacent buttons whose expanded hit areas
                     would overlap. The 44px floor fits inside the py-2 row
                     via -my-2. */}
                <button
                  type="button"
