...
Key extractors for tokens must return a unique value (e.g., `str(uuid.uuid4())`) for invalid tokens, not a fixed string like `'invalid'` - a fixed string lets attackers on a shared IP exhaust the bucket and block legitimate users. Fail closed on the TRUTHINESS of the keyed claim, not on token decodability: `if payload and payload.get('user_id')` - a payload that decodes (valid signature, unexpired) but carries an absent, None, or empty `user_id` still needs the fresh UUID, while a fallback like `str(payload.get('user_id', 'unknown'))` resolves to a fixed key - and because `rate_limit_account`'s cache key has no IP component, that fixed bucket is GLOBAL: replaying one such token 11 times 429s every other user. A `'user_id' in payload` membership test is not enough either - a present-but-None value stringifies to `'None'` and shares one bucket.
...
**Login dummy hash:** the no-user login path runs `hashers.check_password(data.password, _DUMMY_PASSWORD_HASH)` - a module-level hash computed once at import - before returning a 401 byte-identical to the wrong-password path. Without it, user-exists vs wrong-password response times are a user-enumeration oracle. The inactive-account branch joins the same contract: it only executes after the password check succeeds, and it reuses the wrong-password branch's exact `_('Invalid email or password')` detail (same status, same msgid, same handler) - a distinct "account is disabled" detail there is a body oracle telling a credential-holding attacker the account exists and is disabled. Never assert wall-clock timings in tests; verify the two 401 bodies are byte-identical (`self.response.content` + `Content-Type` equality - byte-level assertion is also locale-invariant, unlike msgid text) and leave timing to code review.
